...
class TodoController extends Controller
{
    ...
    {
        $todos = Todo2::where('user_id', Auth::id())-> with('category')->get();
    ...
    public function search(Request $request)
    {
        ...
        $todos = Todo2::where('user_id', Auth::id())
    ...
    {
        try {
            $todo = Todo2::where('user_id', Auth::id())->findOrFail($id);

            $data = $request->validate([
                'title' => 'required|string|max:255',
                'category_id' => 'nullable|exists:categories,id,user_id,' . Auth::id(), // valiadasi category milik user
                'is_done' => 'boolean',
            ]);
        ...
        } catch (\Illuminate\Database\Eloquent\ValidationException $e) {
            return response()->json([
                'status_code' => 404,
